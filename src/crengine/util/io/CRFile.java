...
import static crengine.util.CRString.havaPathStr;
import static crengine.util.CRString.onUFT8;
...
public class CRFile {
        ...
        try {
            ...
            if (type == FileType.local){
                ...
                if (path.charAt(0) == '/' || path.startsWith("./")){
                    throw new CrengineRuntimeException("文件内部路径错误");
                }
                this.path = path;
                this.fileURL = this.getLocalStream(path);
                this.file = new File(this.fileURL.getFile());
            } else {
                throw new CrengineRuntimeException("错误，类型未知");
            }
        } catch (MalformedURLException e) {
            throw new CrengineRuntimeException("创建CRFile时出错", e);
        }
    }
    ...
    public CRFile child(String path){
        if (this.type == FileType.internal) throw new CrengineRuntimeException("内部不支持");
...
//    }
    ...
    public List<CRFile> getExtFiles(String ext){
        if (this.type == FileType.local){
            ...
            try {
                ...
                throw new CrengineRuntimeException("加载内部文件夹列表失败", e);
            }
        } else {
    ...
    public void writeString(String content, boolean append, boolean line) {
        ...
        if(!this.file.isDirectory() && this.type != FileType.internal) {
            ...
            throw new CrengineRuntimeException("文件夹和内部文件无法写入！");
        }
    }
    ...
    public String read(String encoding){
        try {
            if(this.file.isFile() && this.file.exists()){ //判断文件是否存在
                ...
                throw new CrengineRuntimeException("找不到指定的文件");
            }
        } catch (Exception e) {
            throw new CrengineRuntimeException("读取文件内容出错", e);
        }
    }
    ...
    public void deleteFile() {
        if (this.file.exists()) {
            if (this.file.isDirectory()){
                throw new CrengineRuntimeException("无法删除文件夹");
            } else if (this.type == FileType.internal){
                throw new CrengineRuntimeException("内部文件无法删除");
            }

            boolean deleted = this.file.delete();
            if (!deleted) {
                throw new CrengineRuntimeException("无法删除文件: " + this.file.getPath());
            }
        }
    }
}
